...
describe("PluginRequestStatusManager", () => {
	it("updates the status bar on desktop", () => {
		...
		expect(setText).toHaveBeenNthCalledWith(2, "Streaming openai@gpt-5.4");
		expect(setText).toHaveBeenNthCalledWith(3, "Searching the web");
		expect(setText).toHaveBeenNthCalledWith(4, "Waiting for file approval");
		expect(setText).toHaveBeenNthCalledWith(5, "Saving to story.md");
		expect(setText).toHaveBeenNthCalledWith(6, "");
	});
